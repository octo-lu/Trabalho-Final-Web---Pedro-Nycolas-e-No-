...
(function () {
	...
	//Pedro: 
		//Event listener esperando o documento ser preenchido para executar	
	document.addEventListener('DOMContentLoaded', () => {
		console.log('doma')
		//condicional checando a rota atual da página
		if (window.location.pathname == '/site/contacts.html') {

			//DOM buscando os dados do form de cadastro pelo id
			const newsForm = document.getElementById("form-newsletter")
			//Event listener esperando o 'submit' do botão para executar
			newsForm.addEventListener('submit', (event) => {
				event.preventDefault()
				//atribuindo os dados do form pelo DOM
				const nome = document.getElementById("contact-name").value
				const email = document.getElementById("contact-email").value

				//puxando o método POST da api na rota newsletter
				fetch('http://localhost:3000/newsLetter', {
					method: 'POST',
					headers: {
						'Content-type': 'application/json'
					},
					//atribuindo as variáveis no corpo de acordo com o model
					body: JSON.stringify({
						name: nome,
						email: email
					})
				...
				window.alert("Newsletter Assinada!")
			})
		}
		//condicional checando a rota atual da página
		...
		if (window.location.pathname == '/site/index.html') {
			console.log('dom')
			//DOM buscando os dados do form de cadastro pelo id
			const dataForm = document.getElementById("schedule-form")
			//Event listener esperando o 'submit' do botão para executar
			dataForm.addEventListener('submit', (event) => {
				...
				const dataMarcada = document.getElementById("booking-date").value
				const numeroPessoas = document.getElementById("booking-number").value
				
				//puxando o método GET da api na rota schedule
				fetch('http://localhost:3000/schedule').then((res) => {
					...
					//condicional navegando o JSON para verificar se a data sendo submetida tem conflito
					if (data.some(obj => obj.dataMarcada === dataMarcada)) {
						//alerta caso a data já tenha sido usada
						window.alert("Data não disponível")

					  } else {
						//puxando o método POST da api na rota schedule caso a data não tenha conflito
